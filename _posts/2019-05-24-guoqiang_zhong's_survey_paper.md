...
이 논문에서 우리는 전통적인 feature learning과 최신 딥러닝을 포함하여 data representation learning에 관한 연구를 리뷰했습니다.
Feature learning 방법론과 인공신경망의 발달로부터 우리는 딥러닝이 완전히 새로운 것이 아니라는 것을 보았습니다.
딥러닝은 feature learning 연구의 큰 진보의 결과이고 labeled 된 데이터를 많이 사용할 수 있고 하드웨어가 발달한 것도 영향을 미쳤습니다.
그러나 딥러닝의 breakthrough는 인공지능 분야에 큰 영향을 끼쳤을 뿐만 아니라, finance, bioinformatics 같은 많은 도메인의 진보에 큰 향상을 가져왔습니다.

딥러닝의 향후 연구에 대해 우리는 세가지 방향을 제공합니다.
근본적인 이론, 새로운 알고리즘, 어플리케이션입니다.
몇몇 연구자들은 deep neural network를 분석하려고 시도하였습니다.
그러나 딥러닝 어플리케이션과 이론 사이의 갭은 여전히 꽤 큽니다.
비록 많은 딥러닝 알고리즘이 제안되었지만, 대부분은 CNN과 RNN에 기반합니다.
그러므로 실제 세상의 문제를 풀기 위해서 unsupervised 모델이나 transfer learning 모델과 같은 창조적인 딥러닝 알고리즘이 제안될 필요가 있습니다.
게다가 딥러닝 알고리즘은 이미 많은 도메인에서 이용되고 있습니다.
그러나 자연어처리나 컴퓨터 비전과 같은 도전적인 문제를 풀기 위해서 더 세밀한 모델이 만들어지고 적용되어야 합니다.

마지막으로 우리는 딥러닝이 머신러닝의 전부가 아니고 인공지능을 실현하는 한 방향임을 강조합니다.
실제 세상의 문제를 풀기 위해서 지능 데이터 분석을 위한 많은 방법론이 필요합니다.
