...
$$ f(x)=sup_{\alpha \in A}(f_{\alpha}(x)))이고 f_{\alpha}(x)가 convex일 때, \beta = arg sup_{\alpha \in A}(f_{\alpha}(x)))라면 \partial f_{\beta}(x) \in \partial f이다$$
...
GAN은 적대적 프레임워크를 통해 학습 데이터 분포를 잘 모방하는 generator를 만들어내는 것이 목적입니다.
